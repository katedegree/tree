...
export function useTree() {
  ...
  const toggleComplete = useCallback(
    (id: string) => {
      persist({
        ...nodes,
        [id]: { ...nodes[id], completed: !nodes[id].completed },
      });
    },
    [nodes],
  );
  ...
  const toggleCollapsed = useCallback(
    (id: string) => {
      persist({
        ...nodes,
        [id]: { ...nodes[id], collapsed: !nodes[id].collapsed },
      });
    },
    [nodes],
  );
  ...
  const addAction = useCallback(
    (nodeId: string) => {
      ...
      persist({
        ...nodes,
        [nodeId]: { ...node, actions: [...node.actions, action] },
      });
    },
    [nodes],
  );
  ...
  const deleteNode = useCallback(
    (id: string) => {
      ...
      const newNodes = Object.fromEntries(
        Object.entries(nodes).filter(([k]) => !toDelete.has(k)),
      );
