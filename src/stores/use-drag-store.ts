...
interface DragState {
  ...
  setDragState: (state: Partial<Omit<DragState, 'setDragState' | 'resetDragState'>>) => void
  resetDragState: () => void
}
...
export const useDragStore = create<DragState>((set, get) => ({
  ...
  setDragState: (next) => {
    ...
    const ghostMoved =
      ...
      ('ghostInsertIndex' in next && next.ghostInsertIndex !== prev.ghostInsertIndex)
    set({ ...next, ghostKey: ghostMoved ? prev.ghostKey + 1 : prev.ghostKey })
  },
  resetDragState: () =>
    set({ activeId: null, targetId: null, ghostParentId: null, ghostInsertIndex: null }),
}))
